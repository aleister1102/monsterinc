...
func (r *Runner) validateRunState() error {
	if r.httpxRunner == nil {
		return NewError("httpx engine not initialized")
	}

	if r.collector == nil {
		return NewError("result collector not initialized")
	}
...
func (r *Runner) waitForCompletion(ctx context.Context) error {
	...
	case <-done:
		...
		return nil
	case <-ctx.Done():
		result := CheckCancellationWithLog(ctx, r.logger, "HTTPX runner execution")
		if result.Cancelled {
...
func (r *Runner) Run(ctx context.Context) error {
	...
	if err := r.validateRunState(); err != nil {
		return WrapError(err, "failed to validate runner state")
	}
