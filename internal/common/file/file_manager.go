...
func (fm *FileManager) EnsureDirectory(path string, perm fs.FileMode) error {
	if fm.FileExists(path) {
		...
		if err != nil {
			return errors.WrapError(err, "failed to check directory: "+path)
		}
		if !info.IsDir {
			return errors.NewValidationError("path", path, "exists but is not a directory")
		}
		return nil
	}

	if err := os.MkdirAll(path, perm); err != nil {
		return errors.WrapError(err, "failed to create directory: "+path)
	}
...
func (fm *FileManager) WriteFile(path string, data []byte, opts FileWriteOptions) error {
	...
	if opts.CreateDirs {
		...
		if err := fm.EnsureDirectory(dir, 0755); err != nil {
			return errors.WrapError(err, "failed to create parent directories for: "+path)
		}
	}
