...
func (ssv *ScanSummaryValidator) ValidateSummary(summary ScanSummaryData) error {
	if summary.ScanSessionID == "" {
		return NewValidationError("scan_session_id", summary.ScanSessionID, "scan session ID cannot be empty")
	}

	if summary.TotalTargets < 0 {
		return NewValidationError("total_targets", summary.TotalTargets, "total targets cannot be negative")
	}

	if summary.ProbeStats.TotalProbed < 0 {
		return NewValidationError("total_probed", summary.ProbeStats.TotalProbed, "total probed cannot be negative")
	}

	if summary.ProbeStats.SuccessfulProbes < 0 {
		return NewValidationError("successful_probes", summary.ProbeStats.SuccessfulProbes, "successful probes cannot be negative")
	}

	if summary.ProbeStats.FailedProbes < 0 {
		return NewValidationError("failed_probes", summary.ProbeStats.FailedProbes, "failed probes cannot be negative")
	}

	if summary.ScanDuration < 0 {
		return NewValidationError("scan_duration", summary.ScanDuration, "scan duration cannot be negative")
	}
...
func (b *ScanSummaryDataBuilder) Build() (ScanSummaryData, error) {
	if err := b.Validate(); err != nil {
		return ScanSummaryData{}, WrapError(err, "validation failed")
	}
