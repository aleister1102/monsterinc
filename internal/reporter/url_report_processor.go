...
func (r *HtmlReporter) prepareReportData(probeResults []*httpxrunner.ProbeResult, partInfo string) (*ReportPageData, error) {
...
func (r *HtmlReporter) setBasicReportInfo(pageData *ReportPageData, partInfo string) {
	pageData.ReportTitle = r.buildPageTitle(partInfo)
	pageData.GeneratedAt = time.Now().Format("2006-01-02 15:04:05")
	pageData.Config = &ReporterConfigForTemplate{
...
func (r *HtmlReporter) processProbeResults(probeResults []*httpxrunner.ProbeResult, pageData *ReportPageData) {
	...
	displayResults := make([]ProbeResultDisplay, len(probeResults))
	for i, pr := range probeResults {
		if pr == nil {
			continue
		}
		displayResult := ToProbeResultDisplay(*pr)
...
func (r *HtmlReporter) TestProcessProbeResults(probeResults []*httpxrunner.ProbeResult, pageData *ReportPageData) {
	r.processProbeResults(probeResults, pageData)
}

func (r *HtmlReporter) shouldIncludeHostnameInFilter(pr *httpxrunner.ProbeResult) bool {
...
func (r *HtmlReporter) executeAndWriteReport(pageData ReportPageData, outputPath string) error {
...
func (r *HtmlReporter) serializeTableData(probeResults []ProbeResultDisplay) (string, error) {
...
func (r *HtmlReporter) collectFilterData(pr httpxrunner.ProbeResult, hostnames map[string]bool, statusCodes map[int]bool, contentTypes map[string]bool, technologies map[string]bool, urlStatuses map[string]bool) {
...
func (r *HtmlReporter) sortAndAssignFilterData(pageData *ReportPageData, hostnames map[string]bool, statusCodes map[int]bool, contentTypes map[string]bool, technologies map[string]bool, urlStatuses map[string]bool) {
